...
class PublicationControllerTest {
    ...
    private PublicationRequestDTO req(String text) {
        return new PublicationRequestDTO(1, 123, text, OffsetDateTime.now(), 10, 5, 2, null, null, null);
    }
    ...
    @Test
    @DisplayName("GET /alerts/negative-influencers -> 200 OK con lista")
    void findNegativeInfluencerActivity_whenFound_thenReturnsList() throws Exception {
        when(service.findNegativeInfluencerActivity()).thenReturn(List.of(resp(202, "Post de influencer negativo")));
        
        mvc.perform(get(BASE_URL + "/alerts/negative-influencers"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)))
                .andExpect(jsonPath("$[0].publicationApiId", is(202)));
    }
}